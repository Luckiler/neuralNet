...
class neuron {
 ...
 public:
  ...
  bool isConnected =
      false;  /// Used  to make sure all the neurons have at least one input

  double value = 0;  ///<The neuron computed value (0 if not calculated)

  std::vector<neuronConnection>
      connections;  ///<List of all the connections for this neuron
  ...
  std::vector<inputValue>
      inputValues;  ///<List of all the input values with their weight
   ...
   *  to change
   */
  void evolve(int changeChance, int changeQnt);
};
